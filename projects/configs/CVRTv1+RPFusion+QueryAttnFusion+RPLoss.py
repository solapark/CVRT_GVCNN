...
save_dir = '/home/sapark/exp59_ddet/result/CVRTv1+RPFusion+QueryAttnFusion+RPLoss'
img_root = '/home/sapark/VEDet/data/MessyTable/images/' 
reid_input_pickle_dir = '/home/sapark/VEDet_org/data/'
bands, max_freq = 64, 8
num_views = 3
num_query = 900
num_rpn_per_view = 300
...
model = dict(
    ...
    pts_bbox_head=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(type='TMVReidMultiheadCrossAttention', embed_dims=256, num_heads=8, dropout=0.1,
                            attention=dict(type='MultiheadSVAttention', num_views=num_views, num_query=num_query, num_key=num_rpn_per_view, scale_dot_type='query_attn', query_attn='forward', self_view_weight=.2)),
                        dict(type='TMVReidMultiheadCrossAttention', embed_dims=256, num_heads=8, dropout=0.1,
        ...
        loss_intra_view=dict(type='IntraViewLoss', alpha=.2, loss_weight=2.0, pos_thresh=5, neg_thresh=100),
    ),
...
save_reid_pickle = dict(eval_thresh=.1, reid_thresh=.1, visible_thresh=.5, out_dir='/home/sapark/frcnn_keras_original/pickle/messytable/CVRTv1+RPFusion+QueryAttnFusion+RPLoss/reid_output/test')
